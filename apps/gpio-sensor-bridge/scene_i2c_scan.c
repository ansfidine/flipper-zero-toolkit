...
/* Timeout for each I2C probe during scan (ms).
 * 10ms is a good balance: fast enough for scanning ~117 addresses,
 * generous enough to avoid false NACKs on slower devices. */
...
static void scene_i2c_scan_update_widget(GpioSensorBridgeApp* app) {
    ...
    for(uint8_t addr = I2C_SCAN_ADDR_MIN; addr <= I2C_SCAN_ADDR_MAX; addr++) {
        /* Probe: attempt a write to the device address to check readiness */
        if(furi_hal_i2c_is_device_ready(i2c, (addr << 1), I2C_SCAN_PROBE_TIMEOUT_MS)) {
            found++;
